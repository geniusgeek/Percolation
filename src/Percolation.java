public class Percolation {
	...
	byte largestNeigh = getLargestNeighbour(i, j);
	    if (largestNeigh == full)
	        recursiveUpdate(i, j, largestNeigh);
	 }
    }
    

    private void recursiveUpdate(int i, int j, byte largestNeigh) {
    if (i < 1 || j > size - 1 || j < 1 || i > size - 1)
        return;

    if (status[i][j] != full - 1) // return if closed or full
        return;
    if (largestNeigh == (byte) 0)
        largestNeigh = getLargestNeighbour(i, j);
    ...
    if (largestNeigh == full) {
        recursiveUpdate(i + 1, j, (byte) 0);
        recursiveUpdate(i, j + 1, (byte) 0);
        recursiveUpdate(i, j - 1, (byte) 0);
        recursiveUpdate(i - 1, j, (byte) 0);
    }
